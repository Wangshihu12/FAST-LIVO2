 ...
 * @return 是否在视场角范围内
 */
bool VIOManager::isInHorizontalFOV(const V3D &p_w, double max_fov_angle)
{
  // 将世界坐标系下的点转换到相机坐标系
  V3D p_c = new_frame_->w2f(p_w);

  // 计算点在xy平面上的投影向量
  V2D p_xy(p_c[0], p_c[1]);

  // 计算视线方向与x轴的夹角
  double view_angle = std::abs(std::atan2(p_xy.norm(), p_c[2]));

  // 检查是否在视场角范围内
  return view_angle <= max_fov_angle;
}
